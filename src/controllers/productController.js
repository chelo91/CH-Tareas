...
const getProductById = async (req, res) => {
  ...
  // Aca podes hacer if(!result) y con eso alcanzaria para atrapar los valores indefinidos
  if (result == null) {
...
const createProduct = async (req, res) => {
  const productManager = new ProductManager(pathProd)
  // Como sugerencia te podria decir que todas estras transformaciones podes hacerlas
  // en un middleware, de esta forma en el controlador ya tendrias los datos parseados como los necesitas
  ...
  let arrayImages = []
  if (req.files) {
    arrayImages = req.files.map(file => {
      return file.path
    })
  }

  // Aca cuando los campos son iguales, podes poder por ejemplo, title nada mas y te ahorras el title: title ** IGUAL ES CUESTION DE GUSTOS ESTO, SI TE PARECE MEJOR DE LA MANERA QUE LO HICISTE ESTA BIEN
  const newProd = {
    title: title,
